...
var packages = module.exports = {
  ...
  // Look for .meteor/version relative to the appDir argument. If it
  // exists, presumable since we're inside a project load the manifest
  // corresponding to that meteor release version, return the parsed
  // manifest after having loaded all the relevant packages into the
  // cache. If no .meteor/version is found, return null.
  //
  // Options:
  // - versionOverride (String). Used instead of reading .meteor/version
  manifestForProject: function (appDir, options) {
    var self = this;
    var project = require(path.join(__dirname, 'project.js'));
    var releaseVersion = options.versionOverride || project.getMeteorReleaseVersion(appDir);
