    ...
    if (engineVersion !== files.getEngineVersion()) {
      try {
        ...
        var engineTarballFilename =
              "meteor-engine-" + releaseManifest.engine + "-" + "xcxc uname" + "-" + "xcxc uname -m";
        engineTarball = Future.wrap(files.getUrl)({
          url: PACKAGES_URLBASE + "/engines/" + engineTarballFilename,
          encoding: null
        }).wait();
        var engineDir = path.join(warehouse.getWarehouseDir(), 'engines', engineVersion);
        // we don't end up with a corrupt warehouse
        var tmpEngineDir = engineDir + ".tmp" + warehouse._randomToken();
        files.mkdir_p(tmpEngineDir);
        files.extractTarGz(engineTarball, tmpEngioneDir);
