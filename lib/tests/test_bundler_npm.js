...
var _assertCorrectPackageNpmDir = function(deps) {
  ...
  var expectedMeteorNpmShrinkwrapDependencies = _.object(_.map(deps, function(version, name) {
    ...
    // test author.
    //
    // 'from' and 'resolved' were added when we upraded
    // to Node 0.8.18 (5a54a5c79f71b7449c58d7ee809086bc0a542a15)
    _.each(['dependencies', 'from', 'resolved'], function(key) {
      if (actualMeteorNpmShrinkwrapDependencies[name][key])
...
assert.doesNotThrow(function () {
  ...
  var errors = bundler.bundle(appWithPackageDir, tmpOutputDir, {nodeModulesMode: 'skip', release: 'none'});
