...
var deploy_app = function (url, app_dir, opt_debug,
  ...
  with_password(parsed_url.hostname, function (password) {
    ...
    if (opt_set_password)
      get_new_password(function (set_password) {
        ...
        bundle_and_deploy(app_dir, bundleOptions, deployOptions);
      });
    else
      bundle_and_deploy(app_dir, bundleOptions, deployOptions);
  });
...
var bundle_and_deploy = function (app_dir, bundleOptions, deployOptions) {
...
var with_password = function (site, callback) {
  ...
  // XXX we've been using `inFiber` as needed, but I'd really love to
  // just have all calls from the event loop get wrapped in a
  // fiber. -Avital
...
exports.bundle_and_deploy = bundle_and_deploy;
