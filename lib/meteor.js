  ...
  Commands.push({
    ...
    func: function (argv) {
      ...
      var opt = require('optimist')
            .alias('port', 'p').default('port', 3000)
            .describe('port', 'Port to listen on. NOTE: Also uses port N+1 and N+2.')
            .describe('version', 'Meteor release version to test.')
            .describe('deploy', 'Optionally, specify a domain to deploy to instead of running locally.')
            .usage(
              "Usage: meteor test-package [options] [comma delimited packages]\n" +
                "XXX\n");
      ...
      // XXX should find latest release if not specified
      var releaseVersion = new_argv['release-version'] || '0.1';
      ...
      if (new_argv._[1]) {
        ...
        var manifest = packages.manifestForReleaseVersion(releaseVersion);
        testPackages = _.keys(packages.list(manifest));
      }

      // XXX implement --deploy. Need to refactor some of deploy.js to make this possible
      // since the bundler options are created within bundle_and_deploy

      var bundle_opts = {
        nodeModulesMode: 'symlink',
        testPackages: testPackages,
        versionOverride: releaseVersion,
        noMinify: true // curiously, if this isn't set we get "RangeError: Maximum call stack size exceeded" during minification
      };
      app_dir = path.join(__dirname, 'test-runner-app');
      runner.run(app_dir, bundle_opts, new_argv.port);
    }
  });
