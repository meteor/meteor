...
Fiber(function () {
  ...
  var engineSpringboard = function () {
    ...
    if (argv.release) {
      ...
      var appDir = files.find_upwards(files.is_app_dir);
      if (appDir) {
        release = getMeteorReleaseVersion(appDir);
        if (!release) {
        ...
        // no springboarding needed. we're already running the latest
        // engine
        return;
      }
    }

    var releaseManifest = warehouse.releaseManifestByVersion(release);
    if (releaseManifest.engine !== files.getEngineVersion()) {
      console.log('xcxc springboarding to', releaseManifest.engine);
      // xcxc run it without redirecting stdout
    }

    // xcxc store the release version in some global state, used by all commands.
    // xcxc while at it, eliminate releaseVersion()?
  };
  ...
  var main = function() {
    ...
    if (argv.version) {
      ...
      // releaseVersion() might be null if we are running from a
      // checkout with an empty warehouse.  XXX should we do something
