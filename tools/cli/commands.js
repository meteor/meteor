...
const { blue, green, purple, red, yellow } =
...
export function parseServerOptionsForRunCommand(options, runTargets) {
  ...
  if (mobileServerOption) {
    ...
    const isRunOnDeviceRequested = _.any(
      runTargets,
      (runTarget) => runTarget.isDevice
    );
    parsedMobileServerUrl = detectMobileServerUrl(
      parsedServerUrl,
...
function filterWebArchs(webArchs, excludeArchsOption, appDir, options) {
  ...
  if (isBuildMode) {
    // Build Mode
    const isModernOnlyPlatform =
      platforms.includes("modern") && !platforms.includes("legacy");
    if (isModernOnlyPlatform) {
      webArchs = webArchs.filter((arch) => arch !== "web.browser.legacy");
    }
    const hasCordovaPlatforms =
    ...
    // Dev & Test Mode
    const isCordovaDev = (options.args || []).some((arg) =>
      ["ios", "ios-device", "android", "android-device"].includes(arg)
    );
    if (!isCordovaDev) {
      const excludeArchsOptions = excludeArchsOption
        ? excludeArchsOption.trim().split(/\s*,\s*/)
        : [];
      const hasExcludeArchsOptions = (excludeArchsOptions?.length || 0) > 0;
      const hasModernArchsOnlyEnabled =
        appDir && getMeteorConfig()?.modern?.webArchOnly !== false;
      ...
      const automaticallyIgnoredLegacyArchs =
      ...
      if (hasExcludeArchsOptions || automaticallyIgnoredLegacyArchs.length) {
        const excludeArchs = [
          ...excludeArchsOptions,
          ...automaticallyIgnoredLegacyArchs,
        ];
...
main.registerCommand(
  ...
  async function (options) {
    ...
    if (release.current.isCheckout()) {
      var gitLog = (
        await utils.runGitInCheckout("log", "--format=%h%d", "-n 1")
      ).trim();
...
main.registerCommand(
  Object.assign({ name: "run" }, runCommandOptions),
  doRunCommand
);
...
async function doRunCommand(options) {
  ...
  if (release.explicit) {
    if (release.current.name !== projectContext.releaseFile.fullReleaseName) {
      console.log(
        "=> Using %s as requested (overriding %s)",
  ...
  let cordovaRunner;
  const shouldDisableCordova = Boolean(
  ...
  return runAll.run({
    ...
    once: options.once,
    noReleaseCheck:
      options["no-release-check"] || process.env.METEOR_NO_RELEASE_CHECK,
...
main.registerCommand(
  Object.assign({ name: "debug" }, runCommandOptions),
  function (options) {
...
main.registerCommand(
  ...
  async function (options) {
    ...
    const setup = async () => {
      ...
      const r = await prompt([
        ...
        },
        {
          ...
          choices: AVAILABLE_SKELETONS.map((skeleton) => {
            return `${capitalizeFirstLetter(skeleton)} # ${
              SKELETON_INFO[skeleton]
            }`;
          }),
          default() {
            return `${capitalizeFirstLetter(DEFAULT_SKELETON)} # ${
              SKELETON_INFO[DEFAULT_SKELETON]
            }`;
          },
          filter(val) {
    ...
    var { appPathAsEntered, skeleton } = await setup();
    ...
    const setupExampleByURL = async (url) => {
      ...
      const errorMessage =
        errClone && typeof errClone === "string" ? errClone : errClone?.message;
    ...
    const copyFromLocalSkeleton = async () => {
      await files.cp_r(skeletonPath, appPath, {
        transformFilename: function (f) {
          return transform(f);
        },
        transformContents: function (contents, f) {
          ...
          if (/packages/.test(f)) {
            ...
            if (options.prototype) {
              ...
              return Buffer.from(
                contents.toString().replace(/~prototype~/g, "")
              );
            }
          }
          if (/(\.html|\.[jt]sx?|\.css|\.coffee)/.test(f)) {
    ...
    const useLocalSkeleton =
      files.exists(skeletonPath) || options.prototype || release.explicit;
    if (useLocalSkeleton) {
      // Use local skeleton
  ...
  async function (options) {
    Console.error(
      "This command has been deprecated in favor of " +
        Console.command("'meteor build'") +
        ", which allows you to " +
        "build for multiple platforms and outputs a directory instead of " +
        "a single tarball. See " +
        Console.command("'meteor help build'") +
        " " +
...
var buildCommand = async function (options) {
  ...
  if (!serverOnly) {
    ...
    if (process.platform !== "darwin" && cordovaPlatforms.includes("ios")) {
      ...
      Console.warn(
        "Currently, it is only possible to build iOS apps \
on an OS X system."
      );
    }
    ...
    if (!_.isEmpty(cordovaPlatforms)) {
      ...
      parsedMobileServerUrl = parseMobileServerOption(
        mobileServerOption,
        "server"
      );
  ...
  if (selectedPlatforms) {
    const filteredArchs = baseWebArchs.filter((arch) =>
      selectedPlatforms.includes(arch)
    );
...
${Console.command("meteor build ../output")}`,
  ...
  if (!options.directory) {
    await main.captureAndExit("", "creating server tarball", async () => {
      try {
        var outputTar = options._bundleOnly
          ? outputPath
...
${displayNameForPlatform(platform)}`,
          ...
          async () => {
            ...
            if (platform === "ios") {
              ...
              await cordovaProject.buildForPlatform(platform, {
                ...buildOptions,
...
`,
              ...
              const apkPath = files.pathJoin(
                buildPath,
                `app/build/outputs/${packageType}/${
                  options.debug ? "debug" : "release"
                }`,
                options.debug
                  ? `app-debug.${packageExtension}`
                  : `${packageName}.${packageExtension}`
              );
...
`,
  ...
  const npmShrinkwrapFilePath = files.pathJoin(
    bundlePath,
    "programs/server/npm-shrinkwrap.json"
  );
...
main.registerCommand(
  ...
  async function (options) {
    ...
    if (packageDir) {
      ...
      await main.captureAndExit(
        "=> Errors while setting up package:",
...
main.registerCommand(
  ...
  async function (options) {
    ...
    if (options.args.length === 0) {
      ...
      var mongoPort = await findMongoPort(
        files.pathJoin(options.appDir, ".meteor", "local", "db")
      );
...
to this command.`);
    ...
    if (options.url) {
      ...
      await runMongo.runMongoShell(mongoUrl, (err) => {
     ...
     `);
        ...
        if (err.code === "ENOENT") {
          console.log(
            red`The 'mongosh' command line tool was not found in your PATH.`
          );
...
main.registerCommand(
  ...
  async function (options) {
    if (options.args.length !== 0) {
      Console.error(
        "'meteor reset' command only affects the local project cache."
      );
      Console.error();
      Console.error("To remove also the local database use");
      Console.error(
        Console.command("meteor reset --db"),
        Console.options({ indent: 2 })
      );
      Console.error();
      Console.error("To reset a deployed application use");
      Console.error(
        Console.command("meteor deploy --delete appname"),
        Console.options({ indent: 2 })
      );
      Console.error("followed by");
      Console.error(
        Console.command("meteor deploy appname"),
        Console.options({ indent: 2 })
      );
      return 1;
    }
    ...
    if (process.env.MONGO_URL) {
      Console.info(
        "As a precaution, meteor reset only clears the local database that is " +
          "provided by meteor run for development. The database specified with " +
          "MONGO_URL will NOT be reset."
      );
    }

    const resetMeteorNpmCachePromise = options["skip-cache"]
      ? Promise.resolve()
    ...
    const rspackAppContexts = rspackHelpers.getRspackAppContexts(
      options.appDir
    );
    const resetRspackPromises = rspackAppContexts.map((contextPath) =>
      files.rm_recursive_async(contextPath)
    );
    ...
    if (options.db) {
      ...
      var findMongoPort = require("../runners/run-mongo.js").findMongoPort;
      var isRunning = !!(await findMongoPort(
        files.pathJoin(options.appDir, ".meteor", "local", "db")
      ));
      if (isRunning) {
    ...
    var allExceptDb = files
      .getPathsInDir(files.pathJoin(".meteor", "local"), {
        cwd: options.appDir,
        maxDepth: 1,
      })
  ...
  if (options.password) {
    Console.error(
      "Setting passwords on apps is no longer supported. Now there are " +
        ...
        Console.command("'meteor authorized'") +
        " command."
    );
    return 1;
  }
...
main.registerCommand(
  ...
  async function (options) {
    if (
      Object.keys(_.pick(options, "add", "remove", "transfer", "list")).length >
      1
    ) {
      Console.error(
...
testCommandOptions = {
  ...
  options: {
    ...
    // --parallel uses the default concurrency (4).
    // --parallel=N uses N concurrent package-groups.
    parallel: { type: String, implicitValue: "4" },

    // Comma-separated list of packages that must run serially (one at a time,
    // after all parallel-safe packages finish) even when --parallel is active.
    // Defaults to "mongo" when --parallel is used, since mongo tests share
    // global DB state and can fail when interleaved with other packages.
    "serial-packages": { type: String },
  },
};
...
main.registerCommand(
  Object.assign({ name: "test-packages" }, testCommandOptions),
  function (options) {
...
async function doTestCommand(options) {
  ...
  if (options.parallel) {
    ...
    if (!isNaN(concurrency) && concurrency > 0) {
      process.env.TINYTEST_PARALLEL = String(concurrency);
    }
    // When --parallel is active, default serial packages to "mongo" so that
    // mongo tests, which share global DB state, always run after all other
    // parallel-safe packages finish (one at a time).
    // The caller can override with --serial-packages='' to disable or supply
    // a different comma-separated list.
    const serialPkgs =
      options["serial-packages"] !== undefined
        ? options["serial-packages"]
        : process.env.TINYTEST_SERIAL_PACKAGES || "mongo";
    if (serialPkgs) {
      process.env.TINYTEST_SERIAL_PACKAGES = serialPkgs;
    }
  }
  ...
  const runTargets = parseRunTargets(
    _.intersection(Object.keys(options), [
      "ios",
      "ios-device",
      "android",
      "android-device",
  ...
  if (options["test-packages"]) {
    ...
    if (!options["driver-package"]) {
      throw new Error(
        "You must specify a driver package with --driver-package"
      );
    }
    ...
    function prepareCordovaProject() {
      return main.captureAndExit("", "preparing Cordova project", async () => {
        ...
        await projectContext.platformList.write(
          cordovaRunner.platformsForRunTargets
        );
        await cordovaRunner.checkPlatformsForRunTargets();
      });
    }
  ...
  return await runTestAppForPackages(
    projectContext,
        ...
        await buildmessage.enterJob(
          ...
          async function () {
            ...
            if (p.indexOf("/") === -1) {
              ...
              var version = await projectContext.localCatalog.getLatestVersion(
                p
              );
              if (!version) {
...
var runTestAppForPackages = async function (projectContext, options) {
  ...
  buildOptions.webArchs = filterWebArchs(
    webArchs,
    options["exclude-archs"],
...
main.registerCommand(
  ...
  async function (options) {
    ...
    if (!options["force-online"]) {
      try {
        await require("../utils/http-helpers.js").getUrl(
          "http://www.google.com/"
        );
      } catch (e) {
...
const sanitizeBoolAnswer = (string) => {
  ...
  Console.error(
    yellow(
      "it should be either (y)es or (n)o or just press enter to accept the default value"
    )
  );
  throw new main.ExitWithCode(2);
};
...
const checkScaffoldName = (scaffoldName) => {
  ...
  if (allNonWordRegex.test(scaffoldName)) {
    Console.error(red("You must provide a valid name for your model."));
    Console.error(
      yellow("Model names should not contain special characters except _ and -")
    );
    throw new main.ExitWithCode(2);
  }
};
  ...
  async function (options) {
    ...
    const setup = async (arg0) => {
      if (arg0 === undefined) {
        ...
        const scaffoldName = await ask(
          `What is the name of your ${yellow`model`}? `
        );
        checkScaffoldName(scaffoldName);
        const areMethods = await ask(
          `There will be methods [${green`Y`}/${red`n`}]? press enter for ${green`yes`}  `
        );
        const methods = sanitizeBoolAnswer(areMethods);
        const arePublications = await ask(
          `There will be publications [${green`Y`}/${red`n`}]? press enter for ${green`yes`}  `
        );
        const publications = sanitizeBoolAnswer(arePublications);
        const path = await ask(
          `Where it will be placed? press enter for ${yellow`./imports/api/`} `
        );
        return {
          isWizard: true,
          scaffoldName,
      ...
      const { path, methods, publications } = options;
     ...
     * @type{string}
     */
    const { isWizard, scaffoldName, path, methods, publications } = await setup(
      args[0]
    );
    ...
    const userTransformFilenameFn = (filename) => {
      ...
      if (typeof replaceFn !== "function") {
        Console.error(
          red`You must provide a valid function transformFilename.`
        );
        Console.error(
          yellow`The function should be named transformFilename and should be exported.`
        );
        throw new main.ExitWithCode(2);
      }
    ...
    const userTransformContentsFn = (contents, fileName) => {
      ...
      if (typeof replaceFn !== "function") {
        Console.error(
          red`You must provide a valid function transformContents.`
        );
        Console.error(
          yellow`The function should be named transformContents and should be exported.`
        );
        throw new main.ExitWithCode(2);
      }
    ...
    if (!rootFiles.includes(".meteor")) {
      ...
      Console.error(
        yellow`You can create a new Meteor project with 'meteor create'`
      );
      throw new main.ExitWithCode(2);
    }
    ...
    await files.cp_r(assetsPath(), files.pathResolve(scaffoldPath), {
      ...
      transformContents: function (contents, fileName) {
        if (options.replaceFn)
          return userTransformContentsFn(contents.toString(), fileName);
    ...
    const mainJsPath = packageJson?.meteor?.mainModule?.server
...
main.registerCommand(
  ...
  function (options) {
    Console.warn();
    Console.warn(
      "The 'meteor admin get-machine' command has been disabled and",
      "the build farm has been discontinued."
    );
    Console.warn();
    Console.info(
      "As of Meteor 1.4, packages with binary dependencies are",
    ...
    Console.info();
    Console.info(
      "To see the requirements for this compilation step,",
...
main.registerCommand(
  ...
  function (options) {
    ...
    Console.info(
      p("ething") + " " + p("port") + " " + p("changed") + " " + p("args")
    );
    if (options.url) {
      Console.info("url");
    }
...
const setupBenchmarkSuite = async (profilingPath) => {
  ...
  const parsedGitVersion = semver.coerce(
    okGitVersion.match(/\d+\.\d+\.\d+/)?.[0] || ""
  )?.version;
  ...
  if (hasTar) {
    ...
    if (!errTarClone) {
      ...
      Console.warn(
        "Tar-based cloning failed. Will attempt standard git clone..."
      );
...
async function doBenchmarkCommand(options) {
  ...
  const meteorOptions = args.filter(
    (arg) => !["--size-only", "--size", "--build"].includes(arg)
  );

  const profilingCommand = [
    `${meteorSizeEnvs.join(" ")} ${profilingPath}/scripts/monitor-bundler.sh ${
      projectContext.projectDir
    } ${new Date().getTime()} ${meteorOptions.join(" ")}`.trim(),
  ].join(" && ");
