...
const { blue, bold, cyan, dim, green, purple, red, yellow } =
...
const SKELETON_INFO = {
  ...
  "typescript-tailwind":
    "To create an app using TypeScript, React, and Tailwind",
  ...
  async function (options) {
    ...
    if (options.package) {
      ...
      if (options.from || options["from-dir"] || options["from-branch"]) {
        Console.error(
          "Package creation does not support --from, --from-dir, or --from-branch."
        );
        Console.error();
        throw new main.ShowUsage();
      }
    ...
    if (options.list) {
      try {
        ...
        examples.forEach((ex, i) => {
          ...
          const repoUrl =
            ex.repositoryUrl ||
            `${EXAMPLES_REPO}/tree/${EXAMPLES_BRANCH}/${ex.internalPath}`;
          if (ex.demo) {
    ...
    if ((options["from-dir"] || options["from-branch"]) && !options.from) {
      Console.error(
        "--from-dir and --from-branch require --from to specify the source repository."
      );
      return 1;
    }
...
main.registerCommand(
  ...
  async function (options) {
    ...
    if (!hasTsConfig && !hasJsConfig) {
      Console.info(
        "=> No tsconfig.json or jsconfig.json found. Nothing to do."
      );
      return 0;
    }
