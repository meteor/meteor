...
import { optimisticReadFile, optimisticHashOrNull } from '../fs/optimistic';
  ...
  function(packageSource, options) {
          ...
          var buildResult = buildJsImage({
            ...
            npmDir: files.pathResolve(
              files.pathJoin(
                ...
                '.npm',
                'plugin',
                colonConverter.convert(info.name)
              )
            ),
          });
    ...
    packageSource.architectures.forEach(sourceArch => {
      sourceArch.uses.forEach(use => {
        if (
          !use.weak &&
          isIsobuildFeaturePackage(use.package) &&
...
compiler.lint = Profile(
  ...
  function(packageSource, options) {
    ...
    _.each(packageSource.architectures, function(architecture) {
      ...
      if (
        !options.includeCordovaUnibuild &&
...
compiler.getMinifiers = function(packageSource, options) {
  ...
  ['js', 'css'].forEach(function(ext) {
    ...
    if (plugins.length > 1) {
      ...
      buildmessage.error(
        packages.join(', ') +
          ': multiple packages registered minifiers for extension "' +
          ext +
          '".'
      );
    }
...
var lintUnibuild = function({ isopack, isopackCache, sourceArch }) {
  ...
  const unibuild = _.find(isopack.unibuilds, unibuild =>
    archinfo.matches(unibuild.arch, sourceArch.arch)
  );
  ...
  const { sources } = sourceArch.getFiles(
    sourceProcessorSet,
    unibuild.watchSet
  );
...
api.addAssets('${relPath}', 'client').`
    ...
    var declaredExports = _.map(inputSourceArch.declaredExports, function(
      symbol
    ) {
    ...
    if (!process.env.METEOR_FORCE_PORTABLE) {
      ...
      if (
        process.env.METEOR_ALLOW_NON_PORTABLE ||
...
function runLinters({
  ...
  // unibuild.
  const whichArch =
    inputSourceArch.arch === 'os' ? archinfo.host() : inputSourceArch.arch;
  ...
  Object.values(sources).forEach(sourceItem => {
    ...
    if (
      classification.type !== 'meteor-ignore' &&
...
export function getActivePluginPackages(
  isopk,
  { uses, isopackCache, pluginProviderPackageNames, pluginProviderWatchSet }
) {
