...
Object.assign(Module.prototype, {
  ...
  useAsyncClosure: function () {
    return this.files.some(f => f.useAsyncClosure());
  },
  ...
  _chunkifyEagerRequires(chunks, moduleCount, sourceWidth) {
    ...
    const headerAsyncClosure = this.useAsyncClosure();
    _.each(this.files, file => {
      if (file.bare) {
    ...
    if (eagerModuleFiles.length > 0) {
      _.each(eagerModuleFiles, file => {
        ...
        const withAwaitRequire = headerAsyncClosure && file.useAsyncClosure();
        chunks.push(
          file.mainModule ? "\nvar " + exportsName + " = " : "\n",
          `${withAwaitRequire ? "await " : ""}require(`,
...
export function File(inputFile, arch) {
  ...
  // If true, wrap this individual file in an async closure.
  self.isAsync = !!inputFile.isAsync;
...
Object.assign(File.prototype, {
  ...
  useAsyncClosure() {
    return !!(this.bundleArch.includes("os") && this.isAsync)
  },

  _getClosureHeader() {
    const asyncClosure = this.useAsyncClosure();
    ...
    return `(${asyncClosure ? "await (async " : ""}function(){`;
  },

  _getClosureFooter() {
    if (this.meteorInstallOptions) return "}";

    return `}).call(this)${this.useAsyncClosure() ? ")" : ""};\n`
  },
...
var getHeader = function (options) {
  ...
  const asyncHeader = options.module.useAsyncClosure();
  chunks.push(
...
//  - isAsync: if true, the file is wrapped in a promise.
