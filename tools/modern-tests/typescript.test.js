    ...
    testMeteorRspackBundler({
      ...
      customAssertions: {
        ...
        afterRun: async ({ result, tempDir }) => {
          ...
          // Verify the types directory exists
          await assertFileExist(tempDir, ".meteor/local/types");

          // Verify packages.d.ts was generated and contains core package declarations
          await assertFileExist(tempDir, ".meteor/local/types/packages.d.ts", {
          ...
          await assertFileExist(tempDir, ".meteor/local/types/packages.d.ts", {
            content: "declare module 'meteor/accounts-base'",
          });

          // Verify per-package .d.ts files exist in the packages subdirectory
          await assertFileExist(
            tempDir,
            ".meteor/local/types/packages/random.d.ts"
          );
          await assertFileExist(
            tempDir,
            ".meteor/local/types/packages/accounts-base.d.ts"
          );
        },
...
export async function waitForTypeScriptEnvs(outputLines, options = {}) {
  ...
  if (options.isTsxEnabled) {
    await waitForMeteorOutput(outputLines, /.*isTsxEnabled:.*true.*/, options);
  }
}
