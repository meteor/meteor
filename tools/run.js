...
var logToClients = function (msg) {
  ...
  _.each(msg, function (val, key) {
    if (key === "stdout")
      ...
      Log.process({message: val, level: 'info'});
  });
};
...
exports.run = function (context, options) {
  ...
  var startWatching = function (watchSet) {
    if (process.env.METEOR_DEBUG_WATCHSET)
      Log.process({message: JSON.stringify(watchSet, null, 2), level: 'info'});
  ...
  var restartServer = inFiber(function () {
    ...
    if (files.usesWarehouse() && !context.userReleaseOverride &&
      ...
      if (newAppRelease !== context.appReleaseVersion) {
        Log.process({message: "Your app has been updated to Meteor %s from " +
                      "Meteor %s.\nRestart meteor to use the new release." +
                      newAppRelease + context.appReleaseVersion, level: 'error'});
        process.exit(1);
      }
    }
    ...
    if (firstRun) {
      Log.process({message: "Meteor server running on: " + rootUrl, level: 'info'});
      firstRun = false;
      lastThingThatPrintedWasRestartMessage = false;
    } else {
      if (lastThingThatPrintedWasRestartMessage) {
  ...
  var launch = function () {
    Status.mongoHandle = mongo_runner.launch_mongo(
      ...
      function (code, signal, stderr) { // On Mongo dead
        ...
        Log.process({message: stderr + "Unexpected mongo exit code " + code + ". Restarting.\n", level: 'info'});
        ...
        if (mongoErrorCount >= 3) {
          ...
          Log.process({message: "Can't start mongod\n", level: 'info'});
          if (explanation)
            Log.process({message: explanation.longText, level: 'info'});
          if (explanation === mongoExitCodes.EXIT_NET_ERROR)
            Log.process({message: "\nCheck for other processes listening on port " +
              mongoPort + "\nor other meteors running in the same project.", level: 'info'});
          if (!explanation && /GLIBC/i.test(stderr))
            Log.process({message: "\nLooks like you are trying to run Meteor on an old Linux " +
                        ...
                        "to the latest version.", level: 'info'});
          process.exit(1);
        }
        if (mongoErrorTimer)
