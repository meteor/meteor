...
var startProxy = function (outerAddr, innerAddr, callback) {
  ...
  var server = http.createServer(function (req, res) {
    ...
    var proxyIt = function () {
      // XXX YYY
      proxy.web(req, res, {target: 'http://' + innerAddr});
    };
    if (Status.listening) {
  ...
  server.on('upgrade', function(req, socket, head) {
    var proxyIt = function () {
      // XXX YYY
      proxy.ws(req, socket, head, { target: 'http://' + innerAddr});
    };
    if (Status.listening) {
      // server is listening. things are hunky dory!
      proxyIt();
  ...
  server.on('error', function (err) {
    if (err.code == 'EADDRINUSE') {
      process.stderr.write("Can't listen on port "
                           + ports.portFromAddr(outerAddr)
                           + ". Perhaps another Meteor is running?\n");
      process.stderr.write("\n");
      process.stderr.write("Running two copies of Meteor in the same application directory\n");
      process.stderr.write("will not work. If something else is using port "
                           + ports.portFromAddr(outerAddr) + ", you can\n");
  ...
  server.listen(ports.portFromAddr(outerAddr),
                ports.ipFromAddr(outerAddr),
                callback);
};
...
exports.run = function (context, options) {
  ...
  var mongoUrl = process.env.MONGO_URL ||
        ("mongodb://" + options.mongoAddr + "/meteor");
  var firstRun = true;
  ...
  var restartServer = inFiber(function () {
    ...
    var rootUrl = process.env.ROOT_URL ||
          ports.prettyUrlFromAddr(options.mainAddr);
    if (firstRun) {
    ...
    serverHandle = startServer({
      ...
      mainAddr: options.mainAddr,
      appAddr: options.appAddr,
      mongoUrl: mongoUrl,
      rootUrl: rootUrl,
      library: context.library,
        ...
        if (mongoErrorCount >= 3) {
          ...
          if (explanation === mongoExitCodes.EXIT_NET_ERROR)
            console.log("\nCheck for other processes listening on port "
                        + ports.portFromAddr(options.mongoAdrr)
  ...
  // XXX YYY
  startProxy(options.mainAddr, options.appAddr, function () {
