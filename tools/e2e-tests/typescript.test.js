    ...
    customAssertions: {
      ...
      afterRun: async ({ result, tempDir }) => {
        ...
        const serverRuntimePath = path.join(
          tempDir,
          'build/main-dev/server-meteor.js'
        );
        const serverRuntimeBeforeIdle = fs.readFileSync(
          serverRuntimePath,
          'utf8'
        );
        await wait(2_000);
        const serverRuntimeAfterIdle = fs.readFileSync(
          serverRuntimePath,
          'utf8'
        );
        expect(serverRuntimeAfterIdle).toBe(serverRuntimeBeforeIdle);
        expect(
          result.outputLines.filter(line =>
            line.includes('Server modified -- restarting')
          )
        ).toHaveLength(0);
      ...
      afterRunRebuildServer: async ({ result, tempDir }) => {
        const serverRuntimePath = path.join(
          tempDir,
          'build/main-dev/server-meteor.js'
        );
        ...
        const serverRuntimeBeforeError = fs.readFileSync(
          serverRuntimePath,
          'utf8'
        );
        const errorOutputStartIndex = result.outputLines.length;
        await appendFileContent(tempDir, 'server/main.ts', {
          content: '\nexport const invalidTypeScript: = true;\n',
        });
        await waitForMeteorOutput(
          result.outputLines,
          /\[server-rspack\] compiled with \d+ error/i,
          { startIndex: errorOutputStartIndex },
        );
        await wait(2_000);
        expect(fs.readFileSync(serverRuntimePath, 'utf8')).toBe(
          serverRuntimeBeforeError
        );
        expect(
          result.outputLines.filter(line =>
            line.includes('Server modified -- restarting')
