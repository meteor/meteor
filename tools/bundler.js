...
_.extend(Target.prototype, {
  ...
  minifyJs: function (minifiers) {
    ...
    var lines = [
      'var __meteor__minify__errors = [];',
      'function __meteor__minify__caughtError(filename, err) {',
      '  try {',
      '    var f = null;',
      '    var target = null;',
      ...
      '      if (!f) f = console.log;',
      '    }',
      '    if (f) f.apply(target, ["Error from " + filename + ": ", err]);',
      '  } catch (e) {',
      '    // Ignore: Caught an error while trying to log!',
      '  }',
      '  __meteor__minify__errors.push(err);',
      '};',
      'function __meteor__minifiy__runFile(filename, fn) {',
      '  try {',
      '    fn();',
      '  } catch (err) {',
      '    __meteor__minify__caughtError(filename, err);',
      '  }',
      '};',
      'function __meteor__minify__runAtEnd() {',
      '  if (__meteor__minify__errors) {',
      '    throw new Error("Errors encountered during startup; check log for details");',
      '  }',
      '};'];

    _.each(self.js, function (file) {
      var loc = file.url || '<auto-generated file>';
      var fnName = '__meteor__minify__file__' + file.hash();
      var source = file.contents('utf8');
      lines = lines.concat('function ' + fnName + ' ()  { ',
                           source,
                           '\n;\n',
                           '};',
                           '__meteor__minifiy__runFile("' + loc + '", ' + fnName + ');');
    });
    lines = lines.concat('__meteor__minify__runAtEnd();');

    var allJs = lines.join('\n');
