...
selftest.define("linking to meteor script works correctly on Windows", async function () {
  var location =
  ...
  await selftest.expectEqual(parsedLocation, location);
});

selftest.define("linking to meteor script works correctly on Windows not absolute", async function () {
  var location =
