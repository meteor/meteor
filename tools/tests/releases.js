...
selftest.define("springboard", ['checkout', 'net', 'custom-warehouse'], async function () {
  var s = new Sandbox({
    warehouse: {
      v1: { },
      v2: { recommended: true }
...
selftest.define("writing versions file", ['checkout', 'net', 'custom-warehouse'], async function () {
  var s = new Sandbox({
    warehouse: {
      v1: { recommended: true},
      v2: { recommended: true }
...
selftest.define("download and springboard to pre-0.9.0 release", ['net', 'slow', 'custom-warehouse'], async function () {
  var s, run;
  ...
  if (process.platform === "win32") {
    ...
    await run.match("THIS IS A FAKE RELEASE ONLY USED TO TEST ENGINE SPRINGBOARDING");
  }
