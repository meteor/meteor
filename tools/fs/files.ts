...
export function getToolsVersion() {
  if (!inCheckout()) {
    const isopackJsonPath = pathJoin(
      getCurrentToolsDir(),
...
export function rm_recursive_async(path: string) {
  return new Promise<void>((resolve, reject) => {
    rimraf(convertToOSPath(path), (err: Error) =>
...
export function cp_r(
  ...
  if (stat.isSymbolicLink()) {
    ...
    writeFile(
      to,
      options.transformContents(readFile(from), pathBasename(from)),
      {
...
// if it exists
export const symlinkWithOverwrite = Profile(
  'files.symlinkWithOverwrite',
  function symlinkWithOverwrite(source: string, target: string) {
...
export function getPathsInDir(
  ...
  return fiberHelpers.noYieldsAllowed(function() {
    ...
    if (!exists(cwd)) {
      throw new Error(
        "Specified current working directory doesn't exist: " + cwd
      );
    }
...
if (!process.env.METEOR_SAVE_TMPDIRS) {
  cleanup.onExit(function() {
    Object.entries(tempDirs)
      .filter(([_, isTmp]) => !!isTmp)
      .map(([dir]) => dir)
      .forEach(dir => {
        delete tempDirs[dir];
        try {
...
function tryExtractWithNativeTar(
  ...
  if (options.forceConvert) {
    return Promise.reject(
      new Error('Native tar cannot convert colons in package names')
    );
  }
...
function tryExtractWithNative7z(
  ...
  if (options.forceConvert) {
    return Promise.reject(
      new Error('Native 7z.exe cannot convert colons in package names')
    );
  }
  ...
  return new Promise((resolve, reject) => {
    spawn(exeOSPath, ['x', '-y', tarGzBasename], spawnOptions)
      .on('error', reject)
      .on('exit', resolve);
  }).then(code => {
    ...
    return new Promise((resolve, reject) => {
      spawn(exeOSPath, ['x', '-y', tarBasename], spawnOptions)
        .on('error', reject)
        .on('exit', resolve);
    }).then(
      code => {
...
export const createTarball = Profile(
  function(_: string, tarball: string) {
...
export const renameDirAlmostAtomically = Profile(
  ...
  (fromDir: string, toDir: string) => {
    ...
    try {
      ...
      if (isErrnoException(e) && e.code === 'EXDEV') {
        ...
        forceCopy = true;
      } else if (isErrnoException(e) && e.code === 'ENOENT') {
...
export const writeFileAtomically = Profile(
  'files.writeFileAtomically',
  function(filename: string, contents: string | Buffer) {
...
export function runJavaScript(
  ...
  return Profile.time('runJavaScript ' + filename, () => {
    ...
    chunks.push(header);
    if (sourceMap) {
      const sourcemapConsumer = Promise.await(
        new sourcemap.SourceMapConsumer(sourceMap)
      );
      chunks.push(
        sourcemap.SourceNode.fromStringWithSourceMap(code, sourcemapConsumer)
      );
    ...
    return buildmessage
      .markBoundary(script.runInThisContext())
      .apply(null, values);
  });
}
...
// Trims leading and trailing whitespace in a project file.
...
export function _getLocationFromScriptLinkToMeteorScript(
...
function wrapFsFunc<TArgs extends any[], TResult>(
  ...
  return Profile('files.' + fnName, function(...args: TArgs) {
    ...
    const finalResult =
      options && options.modifyReturnValue
...
const wrappedCopyFile = wrapDestructiveFsFunc('copyFile', fs.copyFileSync, [
  0,
  1,
]);
export function copyFile(from: string, to: string, flags = 0) {
  ...
  if (stat && stat.isFile()) {
    ...
    // everyone if the original file is executable by owner. (This mode will be
...
export const realpath = wrapFsFunc<[string], string>(
  'realpath',
  fs.realpathSync,
  [0],
  {
...
export const readdir = wrapFsFunc<[string], string[]>(
  'readdir',
  fs.readdirSync,
  [0],
  {
...
export const readdirWithTypes = wrapFsFunc<[string], Dirent[]>(
  'readdirWithTypes',
  dir => {
...
export const appendFile = wrapDestructiveFsFunc(
  'appendFile',
  fs.appendFileSync
);
export const chmod = wrapDestructiveFsFunc('chmod', fs.chmodSync);
export const close = wrapFsFunc('close', fs.closeSync, []);
export const createReadStream = wrapFsFunc(
  'createReadStream',
  fs.createReadStream,
  [0]
);
export const createWriteStream = wrapFsFunc(
  'createWriteStream',
...
export const readlink = wrapFsFunc<[string], string>(
  'readlink',
  fs.readlinkSync,
  [0]
);
export const rmdir = wrapDestructiveFsFunc('rmdir', fs.rmdirSync);
export const stat = wrapFsFunc(
  'stat',
  fs.statSync as (path: PathLike) => Stats,
  [0],
  { cached: true }
);
...
type StatListener = (current: Stats, previous: Stats) => void;
...
export const watchFile = wrapFsFunc(
  'watchFile',
  (filename: string, options: StatWatcherOptions, listener: StatListener) => {
    return (fs.watchFile(filename, options, listener) as any) as StatWatcher;
  },
  [0]
);

export const unwatchFile = wrapFsFunc(
  'unwatchFile',
  (filename: string, listener: StatListener) => {
