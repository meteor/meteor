...
_.extend(File.prototype, {
  ...
  _shouldTranspile: function () {
    return /\.es6?$/.test(this.sourcePath);
  },

  _getTranspiledResult: function () {
    var Babel = isopackets.load('babel')['babel'].Babel;

    // var numCodeLines = this.source.match(/\n/g).length + 1;
    // var averageLineLength = (this.source.length / numCodeLines);

    var babelOptions = {
      sourceMaps: true,
      sourceFileName: this._pathForSourceMap()

      // Babel can either pretty-print its output or use as little
      // whitespace as possible ("compact").  The default is to
      // pretty-print everything under 100K, and use compact mode for
      // everything over, while complaining loudly about it (printing a
      // console message).  Instead, we pretty-print everything unless it
      // is both fairly large and has long lines.
      //
      // The trade-off here is that we want code to normally be readable,
      // but there are cases like `logic-solver/minisat.js` which is a
      // huge, ugly file generated by Emscripten that should not be
      // pretty-printed, for one thing because it actually slows down Node
      // when it goes to compile it!
      // compact: (this.source.length > 50000 && averageLineLength > 100)
    };

    return Babel.transformMeteor(
      this.source,
      babelOptions
    );
  },
  ...
  getPrelinkedOutput: function (options) {
    ...
    if (self._shouldTranspile()) {
      result = self._getTranspiledResult();

      if (self.sourceMap) {
        var smg = SMG.fromSourceMap(new SMC(result.map));
        smg.applySourceMap(new SMC(self.sourceMap), path);
        result.map = smg.toJSON();
      }

    } else if (self.sourceMap) {
      result = {
        code: self.source,
        map: self.sourceMap
      };
      ...
      _.each(lines, function (line, i) {
        ...
        var end = { line: i + 1, column: line.length };
        smg.addMapping({
          original: end,
          generated: end,
          source: path
    ...
    chunks.push(SN.fromStringWithSourceMap(result.code, smc));
