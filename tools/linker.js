...
var transpileFile = function (inputFile) {
  ...
  var numCodeLines = oldCode.match(/\n/g).length + 1;
  var averageLineLength = (oldCode.length / numCodeLines);

  var babelOptions = {
    sourceMaps: true,
    // Babel can either pretty-print its output or use as little whitespace
    // as possible ("compact").  The default is to pretty-print everything
    // under 100K, and use compact mode for everything over, while complaining
    // loudly about it (printing a console message).  Instead, we pretty-print
    // everything unless it is both fairly large and has long lines.
    //
    // The trade-off here is that we want code to normally be readable, but
    // there are cases like `logic-solver/minisat.js` which is a huge, ugly
    // file generated by Emscripten that should not be pretty-printed, for
    // one thing because it actually slows down Node when it goes to compile
    // it!
    compact: (oldCode.length > 50000 && averageLineLength > 100)
  };
