...
`webapp` exposes the [express](https://expressjs.com/) API for
...
> **Note:** `WebApp.connectHandlers` is still available as an alias for backwards
> compatibility, but `WebApp.handlers` is recommended for new code.

{% apibox "WebApp.handlers" %}
{% apibox "handlersCallback(req, res, next)" %}
...
Then using the handlers method described above serve up your static HTML on app-root/ page load as shown below. Since `webapp` now uses express, you can use `express.Router()` directly for routing:
...
We're using express's built-in `Router` for route processing. You can also use any express-compatible middleware package.
