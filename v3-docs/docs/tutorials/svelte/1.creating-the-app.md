...
Starting from Meteor 3.4+, new apps use Rspack by default. It includes built-in Hot Module Replacement (HMR), with `svelte-loader`'s hot reload enabled in the skeleton's `rspack.config.js`. This lets you see changes inside Svelte components as you make them.
