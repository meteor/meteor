...
To add TypeScript syntax support to an existing app that uses Meteor's classic
build stack, install the compiler package:

```bash
meteor add typescript
```

The package transpiles `.ts` and `.tsx` files but does not type-check them. Apps
using the modern build stack transpile application TypeScript through Rspack
and SWC instead. See the [`typescript` package guide](/packages/typescript) for
the differences and supported syntax.

Create a TypeScript application with:

```bash
meteor create --typescript my-app
```

Meteor 3.6 preserves the template's established `@types/meteor` and
`zodern:types` providers. Native package declarations are a separate, explicit
opt-in through `meteor types`. Follow the
[TypeScript Types for Meteor Packages](/cli/using-core-types) guide before
changing providers or copying a `tsconfig.json` between projects.
