...
- [`zodern:types`](https://packosphere.com/zodern/types). Still compatible, automatically providing Meteor types for core and community packages. Meteor 3.6 also offers an explicit native opt-in through `meteor types` — see [Using core types](/cli/using-core-types) — while preserving zodern for projects that do not migrate.
