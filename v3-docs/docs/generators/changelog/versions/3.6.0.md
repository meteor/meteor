...
- **Opt-in MongoDB collation for case-insensitive account lookups**, [PR#14211](https://github.com/meteor/meteor/pull/14211)
  - 🔍 Set `caseInsensitiveCollation` in `Meteor.settings.packages.accounts` to replace the regex prefix-permutation lookup for usernames and emails with an indexed equality query using a MongoDB collation (`{ locale: "en", strength: 2 }` by default, or a custom collation object)
  - 📇 Create the `username_ci` and `emails.address_ci` collation indexes at startup when enabled, and fail with a descriptive `case-insensitive-index-conflict` error when an index with that name already exists with different options
  - 🔁 Keep the regex strategy as the default, so existing applications are unaffected until they opt in; both strategies are covered by a shared parity test suite

  This addresses the [slow `createUser` lookup report](https://forums.meteor.com/t/slow-mongo-query-on-createuser/54879) on the forums. See [Case-insensitive lookups with MongoDB collation](https://docs.meteor.com/api/accounts#accounts-case-insensitive-collation) for the migration checklist.
...
Case-insensitive username and email lookups keep using the regex strategy unless you opt in. To switch to MongoDB collation, review the [migration checklist](https://docs.meteor.com/api/accounts#accounts-case-insensitive-collation) (case-only account collisions, index names, index build time on large collections), then set `"caseInsensitiveCollation": true` under `packages.accounts` in your settings file. Setting it to `false` or removing it restores the regex strategy.
