...
- `ddp-common@1.4.6` splits DDP serialization behind a pluggable serializer: `DDPCommon.setSerializer()` and `getSerializer()` swap the process-wide serializer, `createEJSONSerializer()` builds the default one, and `toWireMessage()` / `fromWireMessage()` convert cleared fields for serializers that encode the wire form generically. EJSON stays the default, the wire format is unchanged, and the server and all its clients must install the same serializer, [PR#14235](https://github.com/meteor/meteor/pull/14235).
