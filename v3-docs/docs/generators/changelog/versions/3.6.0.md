...
- **Add `accounts-webauthn`: security key and passkey authentication**
  - 🔑 Log in without a password with a FIDO2 hardware key or a platform passkey through `Meteor.loginWithWebAuthn`, either by letting the key identify the account or by typing a username or email first, and sign up with only a key through `Accounts.createUserWithWebAuthn`
  - 🛡️ Ask for a security key as a second factor for `accounts-password` and `accounts-passwordless` logins with `Accounts.enableWebAuthnSecondFactor`, completed by `Meteor.loginWithPasswordAndWebAuthn` and `Meteor.passwordlessLoginWithTokenAndWebAuthn`; a user who has also enabled another factor can complete the login with either one
  - 🗝️ Manage keys with `Accounts.registerWebAuthnCredential`, `listWebAuthnCredentials`, `renameWebAuthnCredential` and `removeWebAuthnCredential`, vet registrations with `Accounts.validateWebAuthnRegistration`, audit key changes with `Accounts.onWebAuthnCredentialChange`, and configure the relying party through `Accounts.config({ webauthn })`
  - 🔁 Add `Accounts.registerSecondFactor` to `accounts-base` so any package can contribute a second factor; `accounts-2fa` registers its authenticator code through it, and a user with several factors enabled can complete the login with any one of them
