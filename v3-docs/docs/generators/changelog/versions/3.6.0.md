...
- **Support Rspack 2.x**, [PR#14360](https://github.com/meteor/meteor/pull/14360)
  - ⚡ Require and auto-install `@rspack/core` and `@rspack/cli` 2.2.7 or newer, together with `@rspack/dev-server` 2.2.1, `@swc/core` 1.16.2, `@swc/helpers` 0.5.23, `@rspack/plugin-react-refresh` 2.0.2, `react-refresh` 0.19.0, and `@rsdoctor/rspack-plugin` 1.6.4; update the CoffeeScript loader to `swc-loader` 0.2.7
...
- Upgrade `@meteorjs/swc-core` to 1.16.2 in `babel-compiler` and `standard-minifier-js`, matching the SWC version used by the Rspack integration
...
- Keep dynamic imports working after the browser closes IndexedDB: fall back to network fetching, reopen the cache on later operations, and handle background cache-write failures without unhandled rejections, [PR#14780](https://github.com/meteor/meteor/pull/14780), issue [#14761](https://github.com/meteor/meteor/issues/14761)
...
- Require **Windows 11 or Windows Server 2022 (x64)** to run the bundled MongoDB 8.0 locally. Windows 10, Windows Server 2019, and earlier versions are no longer supported by the bundled database. See [MongoDB 8.0 platform support](https://www.mongodb.com/docs/v8.0/tutorial/install-mongodb-on-windows/#platform-support), [PR#14663](https://github.com/meteor/meteor/pull/14663).
...
- `rspack@1.4.0` requires Rspack 2.x: `@rspack/core` and `@rspack/cli` 2.2.7 or newer, `@rspack/dev-server` 2.2.1 or newer, and `@meteorjs/rspack` 3.0.0. Meteor installs them on the next run when automatic dependency installation is enabled. Custom `rspack.config.js` overrides must follow the [Rspack 1.x migration guide](https://rspack.rs/guide/migration/rspack_1.x).
...
If you use Meteor's bundled local MongoDB on Windows 10, Windows Server 2019, or an earlier version, upgrade to Windows 11 or Windows Server 2022 (x64) before updating Meteor. See the [installation requirements](/about/install#prereqs-os).
...
Apps using Rspack receive `@rspack/core` and `@rspack/cli` 2.2.7, `@rspack/dev-server` 2.2.1, and `@meteorjs/rspack@3.0.0-beta.2` on the next `meteor run`. If automatic dependency installation is disabled, run the `meteor npm install --save-dev ...` command Meteor prints. Review custom `rspack.config.js` overrides against the [Rspack 1.x migration guide](https://rspack.rs/guide/migration/rspack_1.x).
