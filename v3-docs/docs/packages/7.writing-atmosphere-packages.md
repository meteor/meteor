...
### Declaring types with `api.types()`
...
1. **`api.types()`** in `package.js` — recommended going forward.
2. **`package-types.json`** in the isopack — legacy format used by some existing
   Atmosphere packages (`zodern:types`-style). Still fully supported.
...
### `package-types.json` (legacy format)

Existing packages that used `zodern:types` may have a `package-types.json` file.
This format is still recognized:
...
You do not need to migrate immediately — the fallback is permanent.
