...
**New in Meteor 3.x**: Atmosphere packages can bundle TypeScript type declarations directly. When they do, Meteor automatically includes those types in the auto-generated `.meteor/types/packages.d.ts` file.
...
The `.d.ts` file path is relative to the package directory. The file will be included in the published isopack as a server asset so it is available both when the package is used locally and when installed from Atmosphere.
...
::: warning Migrating from `api.addAssets()`
If your package already registers its declaration file with
`api.addAssets('my-package.d.ts', 'server')`, remove that call when adopting
`api.types()`. `api.types()` registers the file as a server asset itself, so
keeping both fails the build with `Duplicate asset file: my-package.d.ts`.
:::
...
bundled with the package as a server asset, preserving the tree structure.
