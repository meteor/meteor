...
Package.onUse(function (api) {
  api.addFiles(
    ["stylesheets/_util.scss", "stylesheets/_variables.scss"],
    "client",
    { isImport: true }
  );
});
...
**New in Meteor 3.x**: Atmosphere packages can bundle TypeScript type declarations
directly. When they do, Meteor automatically includes those types in the
auto-generated `.meteor/local/types/packages.d.ts` file — no extra packages or
manual steps are needed by the consumer.

### Declaring types with `api.types()` _(experimental)_

Use `api.types()` in your `onUse` block to point to your package's main `.d.ts`
file:
...
The `.d.ts` file path is relative to the package directory. The file will be
included in the published isopack as a server asset so it is available both when
the package is used locally and when installed from Atmosphere.
...
Some packages expose multiple entry points (e.g. a `suspense` sub-path). You can
declare each one with the `modules` option:
...
```ts
import { useData } from "meteor/username:my-package"; // main
import { useHook } from "meteor/username:my-package/hooks"; // sub-path
import { useSuspend } from "meteor/username:my-package/suspense"; // sub-path
```

### How Meteor resolves package types (priority order)

When generating `.meteor/local/types/packages.d.ts`, Meteor looks for type
information in this order:

1. **`api.types()`** in `package.js` — new, experimental.
2. **`package-types.json`** in the isopack — how the community has been doing it
   (`zodern:types`-style). Still fully supported and encouraged.
3. **Single `.d.ts` file** in the isopack — if the package contains exactly one
   `.d.ts` resource, Meteor uses it automatically without any configuration.

### `package-types.json` (community standard)

Packages published with `zodern:types` use a `package-types.json` file.
This format continues to work and is still the established approach:
