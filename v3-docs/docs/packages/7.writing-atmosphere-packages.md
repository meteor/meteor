...
Meteor installs these dependencies into the package's `.npm/package` directory and records the exact resolved tree in an `npm-shrinkwrap.json` file next to it. Starting with Meteor 3.6, the transitive versions pinned in that file are preserved when the package is rebuilt without a cached `node_modules/`, so the package installs the same dependency tree on every machine. Meteor writes a `.gitignore` inside `.npm/package` that excludes only `node_modules`, so commit the `npm-shrinkwrap.json` file with your package to keep installs reproducible. Meteor 3.6 also moves this file to `lockfileVersion` 5 and records the declared `Npm.depends` versions in it; the first build after updating reinstalls the package dependencies once and rewrites the file.
...
Starting in Meteor 3.6, Atmosphere packages can declare TypeScript type
metadata directly. Applications that opt in with `meteor types` receive those
declarations under `.meteor/types/`.

### Declaring types with `api.types()` _(experimental)_

Use `api.types()` in your `onUse` block to point to your package's main `.d.ts` file:

```js
Package.onUse(function(api) {
  api.versionsFrom('3.6');
  api.use('ecmascript');
  api.mainModule('my-package.js');

  // Tell Meteor where the TypeScript declarations live.
  api.types('my-package.d.ts');
});
```

The `.d.ts` path is relative to the package directory. Meteor stores the file
as an internal server-architecture resource in the published isopack. It is
available to the native generator but excluded from application runtime assets
and from `zodern:types` legacy `.d.ts` discovery.

Single-file mode accepts a `.d.ts` declaration entry or a `.ts`/`.tsx` source
entry. Other extensions fail the package build instead of being treated as
declarations.

::: warning Preserving a legacy provider
If your package already registers a declaration with `api.addAssets()` and
`package-types.json`, keep that file unchanged for zodern consumers. Register a
separate corrected declaration with `api.types()` when the native surface is
not source-compatible. Do not register the same path with both `api.types()`
and `api.addAssets()`, because that creates a duplicate asset. This separation
prevents a Meteor 3.6 update from silently replacing the legacy provider.
:::

**`my-package.d.ts`** example:
...
Consumers of the package can then write:

```ts
import { greet, version } from 'meteor/username:my-package';
//        ^ typed after the app opts in with meteor types
```

### Sub-path module types

Some packages expose multiple entry points (for example, a `suspense` sub-path). You can declare each one with the `modules` option:
...
Package.onUse(function(api) {
  ...
  api.types('my-package.d.ts', {
    modules: {
      hooks: 'hooks.d.ts',
      suspense: 'suspense.d.ts',
    },
  });
});
```

This lets consumers import from sub-paths:

```ts
import { useData } from 'meteor/username:my-package';
import { useHook } from 'meteor/username:my-package/hooks';
import { useSuspend } from 'meteor/username:my-package/suspense';
```

Module keys are POSIX-style import sub-paths: use `nested/hooks`, not
`nested\\hooks`, and do not use empty, `.` or `..` segments. In declaration
mode every module value must end in `.d.ts`; with a `.ts`/`.tsx` source entry,
every module value must also be a `.ts`/`.tsx` source. Meteor encodes module
keys into collision-free generated filenames, so keys such as `index`, `a/b`,
and `a::b` remain distinct. Those filenames are internal; consumers should
only depend on the `meteor/package/sub-path` module id.

### Declaring a directory of declaration files

If your package is written in TypeScript, you likely generate its declarations
with `tsc` instead of maintaining a single hand-written `.d.ts`. Pass a
directory — marked with a trailing slash — to `api.types()` to bundle the
whole generated tree:

```bash
# from the package directory
tsc --declaration --emitDeclarationOnly --declarationDir dist-types --rootDir ./
```
...
Package.onUse(function (api) {
  ...
  api.types('dist-types/', {
    // Optional; in directory mode all paths are relative to the directory.
    modules: {
      hooks: 'client/hooks.d.ts',
    },
  });
});
```

Every `.d.ts` file under the directory (plus any `.d.ts.map` next to it) is
bundled as an internal server-architecture resource, preserving the tree
structure without exposing it as an application runtime asset.
Because the files ship verbatim, they may freely import each other with
relative paths — exactly as `tsc` emits them — with no wrapping or
post-processing needed.

The main entry defaults to the file mirroring your `api.mainModule` (here
`server/main.ts` → `dist-types/server/main.d.ts`), falling back to
`index.d.ts` at the directory root when there is no main module. Pass the
`entry` option (also relative to the directory) to override:

```js
api.types('dist-types/', { entry: 'exports.d.ts' });
```

In directory mode, `entry` and every `modules` value must stay inside the
declared directory and end in `.d.ts`.

Adding or removing a declaration file in the directory triggers a rebuild
automatically during local development. Remember to re-run `tsc` before
`meteor publish` so the bundled declarations match the published code.

### Publishing a TypeScript package

If you don't want to run `tsc` yourself at all, point `api.types()` straight
at your TypeScript entry file:
...
Package.onUse(function (api) {
  ...
  // A .ts (or .tsx) entry marks the package as TypeScript-authored.
  api.types('index.ts');
});
```

During local development, apps that use the package type-check directly
against its TypeScript sources — no declaration files are ever generated on
your machine.

When you run `meteor publish`, Meteor generates the declarations for you:

1. It runs `tsc --declaration --emitDeclarationOnly` in the package
   directory, emitting the declaration tree into a `.types-build/` folder.
   If the package has a locally installed `typescript` package, that compiler
   is used; otherwise Meteor falls back to the TypeScript bundled with its own
   dev bundle. A package-local `typescript` dependency is therefore optional,
   but pinning one makes declaration output reproducible across Meteor
   releases.
2. A `tsconfig.json` in the package directory is respected (`tsc -p ./`), so
   your `strict`, `target`, and friends apply to the generated declarations.
   Without one, `tsc` compiles the entry, every `options.modules` source, and
   their import closure with its defaults.
3. The package is then published exactly as if you had written
   `api.types('.types-build/')` — a plain directory of declaration files.
   Consumers that run `meteor types` get typed
   `import ... from 'meteor/your:package'` and never see the `.ts` entry.

If `tsc` reports errors, the publish aborts and the compiler output is
printed verbatim; fix the errors (or adjust your `tsconfig.json`) and
publish again.

In this mode any `options.modules` values must also be `.ts`/`.tsx` source
files — their declarations are generated and rewritten the same way. The entry
and module files must already be compiled package sources; `api.types()` records
their type metadata but does not add source files to the package build.

::: warning Keep `node_modules` out of your published package
If you do keep a local `node_modules` directory in the package (for editor
tooling or a pinned `typescript`), add a `.meteorignore` file with a
`node_modules/` line in the package directory. Otherwise the installed dev
dependencies are picked up as lazy sources and ship inside the published
package artifacts.
:::

### How Meteor resolves package types (priority order)

When generating `.meteor/types/`, Meteor looks for type information in this
order:

1. **`api.types()`** in `package.js` — the experimental Meteor 3.6 API.
2. **`package-types.json`** in the isopack — the established community format,
   also used by `zodern:types`.
3. **A single `.d.ts` resource** in the isopack — if there is exactly one and
   neither form of metadata is present, Meteor uses it automatically.

### `package-types.json` (community standard)

Packages published with `zodern:types` use a `package-types.json` file. This format continues to work and is still the established approach:
...
You do not need to migrate to `api.types()` — `package-types.json` is fully supported.
...
`test-in-browser` supplies `jquery` on the client because the released Blaze test UI still needs it. Meteor 3.5.2 briefly dropped that dependency and Meteor 3.6 restores it. If your package tests use jQuery themselves, declare `jquery` in `api.use(...)` inside `Package.onTest` rather than relying on the test driver, so they keep working with a future Blaze version that no longer needs jQuery. Running `meteor test-packages` with `--extra-packages=jquery` also works.
