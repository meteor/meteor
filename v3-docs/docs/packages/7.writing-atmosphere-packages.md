...
Starting with Meteor 3.5.2, `test-in-browser` no longer supplies jQuery transitively. If your package tests use jQuery, add `jquery` to `api.use(...)` in `Package.onTest`, or run `meteor test-packages` with `--extra-packages=jquery`.
