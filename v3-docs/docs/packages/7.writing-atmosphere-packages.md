...
**New in Meteor 3.x**: Atmosphere packages can bundle TypeScript type declarations directly. The explicit `meteor types` command includes them in `.meteor/types/packages.d.ts`.
...
The `.d.ts` file path is relative to the package directory. Meteor stores it as an internal server-architecture resource in the published isopack. It is available to the native generator but excluded from application runtime assets and from zodern's legacy `.d.ts` discovery.
...
not source-compatible. This prevents an update from silently replacing the
legacy provider.
:::
...
1. It runs `tsc --declaration --emitDeclarationOnly` in the package
   ...
   If the package has its own `node_modules/.bin/tsc` (a pinned `typescript`
   dev dependency), that compiler is used; otherwise Meteor falls back to
   the TypeScript bundled with its own dev bundle — **no `typescript`
   dependency is required at all**.
2. A `tsconfig.json` in the package directory is respected (`tsc -p ./`), so
   ...
   Without one, `tsc` compiles the entry file with its defaults.
3. The package is then published exactly as if you had written
   ...
   Consumers get fully typed `import ... from 'meteor/your:package'` with no
   extra setup, and never see the `.ts` entry.
...
files — their declarations are generated and rewritten the same way.
...
When generating `.meteor/types/packages.d.ts`, Meteor looks for type information in this order:

1. **`api.types()`** in `package.js` — new, experimental.
2. **`package-types.json`** in the isopack — how the community has been doing it (`zodern:types`-style). Still fully supported and encouraged.
3. **Single `.d.ts` file** in the isopack — if the package contains exactly one `.d.ts` resource, Meteor uses it automatically without any configuration.
