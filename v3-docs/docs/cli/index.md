...
| `--settings`, `-s <file>`      | Set optional data for Meteor.settings on the server. Alternatively, define default settings inline via the `meteor.settings` field in `package.json`; when both are present, Meteor deep-merges them and the file wins on conflicts |
