...
When running your bundled application in production mode, pass a **full JSON object** containing your settings:

```bash
METEOR_SETTINGS='{"server_only_setting":"foo","public":{"client_and_server_setting":"bar"}}'
```

This replaces all settings at once. In development, use `meteor --settings [file.json]` or `meteor.settings` in `package.json` for reactive updates. To override individual keys at any time, use [`METEOR_SETTINGS_*`](#meteor_settings_path) env vars instead.

Please see the [Meteor.settings](../api/meteor#Meteor-settings) documentation for further information.

## METEOR*SETTINGS*\<PATH\>

(_development, production_)

Override **individual** `Meteor.settings` keys via environment variables, without replacing all settings at once. This is the highest-priority settings source and works in both development and production.

**Format:** `METEOR_SETTINGS_` followed by the dot-path to the setting, using `_` as the path separator, uppercased.

```bash
# Sets Meteor.settings.public.theme = "light"
METEOR_SETTINGS_PUBLIC_THEME=light meteor run

# Sets Meteor.settings.apiurl = "https://api.example.com"
METEOR_SETTINGS_APIURL=https://api.example.com meteor run

# JSON values are parsed automatically
METEOR_SETTINGS_FEATUREFLAGS='{"beta":true}' meteor run
```

**Precedence** (lowest → highest):

1. `meteor.settings` in `package.json` — committed defaults
2. `--settings <file>` / `METEOR_SETTINGS` — per-environment overrides
3. `METEOR_SETTINGS_*` env vars — individual key overrides (this)

All sources are deep-merged, so only the keys you specify are overridden.

::: warning Keys with underscores
Because `_` is the path separator, flat setting keys that contain underscores (e.g. `"my_setting"`) cannot be targeted individually — the `_` is interpreted as a nested path separator. For example, `METEOR_SETTINGS_MY_SETTING=x` would set `{ my: { setting: x } }`, not `{ my_setting: x }`. Use camelCase keys if you need env-var overrides.
:::
