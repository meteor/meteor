...
IF EXIST "%~dp0\.git" (
  ...
  IF NOT EXIST "%~dp0\dev_bundle" (
    PowerShell.exe -executionpolicy ByPass -version 2.0 -file "%~dp0\scripts\windows\download-dev-bundle.ps1"
  )

  rem if dev_bundle is the wrong version, remove it and get a new one
  PowerShell.exe -executionpolicy ByPass -version 2.0 -file "%~dp0\scripts\windows\check-dev-bundle.ps1"
  IF errorlevel 1 (
