// implementation of dynamic scoping, for use on the server
...
// TODO fibers - check this BIG time
if (!isFiberEnabled && !global.getAsyncLocalStorage) {
  throw new Error('No getAsyncLocalStorage instance');
}
const asyncLocalStorage = global.getAsyncLocalStorage;

function initDynamicsNode() {
  let nextSlot = 0;
  ...
  function withValue({ value, func, slot }) {
    ...
    const saved = meteorDynamics[slot];
    try {
  ...
  function withValueWithFibers({ value, func, slot }) {
    Meteor._nodeCodeMustBeInFiber();
    ...
    const saved = currentValues[slot];
    try {
      currentValues[slot] = value;
      return func();
    } finally {
      currentValues[slot] = saved;
    }
  }
  ...
  EVp.withValue = function(value, func) {
    ...
    console.log('withValue PARAMS', params);
    if (isFiberEnabled) {
  ...
  function bindEnvironment({ func, onException, _this }) {
    const dynamics = getStore()._meteor_dynamics;
    const boundValues = dynamics ? dynamics.slice() : [];
    ...
    return function(/* arguments */) {
      ...
      var runWithEnvironment = function() {
        var savedValues = getStore()._meteor_dynamics;
        try {
          // Need to clone boundValues in case two fibers invoke this
          // function at the same time
          updateStore('_meteor_dynamics', boundValues.slice());
          var ret = func.apply(_this, args);
        } catch (e) {
          // note: callback-hook currently relies on the fact that if onException
          // throws and you were originally calling the wrapped callback from
          ...
          updateStore('_meteor_dynamics', savedValues);
        }
        return ret;
      };
...
const BetterPromise = Npm.require('es6-promise');
const self = this;

const promise = new BetterPromise(resolve => {
  resolve();
});
promise.then(() => {
  asyncLocalStorage().run(self, () => {
    initDynamicsNode();
  });
});
