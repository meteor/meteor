...
Google.requestCredential = function (options, credentialRequestCompleteCallback) {
  ...
  Accounts.withLoginServiceConfiguration("google", function (config) {
    if (!config) {
    ...
    // Use Google's domain-specific login page if we want to restrict creation
    // to a particular email domain. (Don't use it if
    // restrictCreationByEmailDomain is a function.) Note that all this does is
    // change Google's UI --- accounts-base/accounts_server.js still checks
    // server-side that the server has the proper email address after the OAuth
    // conversation.
