...
Twitter.requestCredential = function (options, credentialRequestCompleteCallback) {
  ...
  Accounts.withLoginServiceConfiguration("twitter", function (config) {
    if (!config) {
    ...
    // We need to keep credentialToken across the next two 'steps' so we're
    // adding a credentialToken parameter to the url and the callback url that
    // we'll be returned to by oauth provider
