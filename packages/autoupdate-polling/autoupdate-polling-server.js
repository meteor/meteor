...
export const AutoupdatePolling = __meteor_runtime_config__.autoupdate = {
...
// startup.
AutoupdatePolling.autoupdateVersion = null;
AutoupdatePolling.autoupdateVersionRefreshable = null;
AutoupdatePolling.autoupdateVersionCordova = null;
AutoupdatePolling.appId = __meteor_runtime_config__.appId = process.env.APP_ID;
...
function updateVersions(shouldReloadClientProgram) {
  ...
  const {
    ...
    AUTOUPDATE_VERSION = AutoupdatePolling.autoupdateVersion
  } = process.env;

  // Step 2: update __meteor_runtime_config__.autoupdate.versions.
  const clientArchs = Object.keys(WebApp.clientPrograms);
  clientArchs.forEach(arch => {
    AutoupdatePolling.versions[arch] = {
...
Meteor.publish(
  ...
  function (appId) {
    ...
    if (AutoupdatePolling.appId && appId && AutoupdatePolling.appId !== appId)
      return [];
