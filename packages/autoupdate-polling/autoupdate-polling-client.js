...
const clientVersions =
  AutoupdatePolling._clientVersions = // Used by a self-test.
  new ClientVersions();
...
AutoupdatePolling._pollingSubscribe = (delay) => {
  Meteor.clearInterval(lastInterval);
  lastInterval = Meteor.setInterval(() => {
    ...
    xhr.open("GET", `/meteor_autoupdate_polling_clientVersions?arch=${clientArch}`);
    xhr.send();
  }, delay);
};
