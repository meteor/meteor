...
Meteor.loginWithPassword = function (selector, password, callback) {
  ...
  Meteor.apply('beginPasswordExchange', [request], function (error, result) {
    if (error || !result) {
      ...
      error = error || new Error("Geen resultaat van aanroep naar beginPasswordExchange");
    ...
    Accounts.callLoginMethod({
      ...
      validateResult: function (result) {
        if (!srp.verifyConfirmation({HAMK: result.HAMK}))
          throw new Error("Server belazert de boel!");
      },
...
Accounts.changePassword = function (oldPassword, newPassword, callback) {
  if (!Meteor.user()) {
    callback && callback(new Error("Moet ingelogd zijn om het wachtwoord te kunnen veranderen."));
    return;
  }
  ...
  if (!oldPassword) {
    Meteor.apply('changePassword', [{srp: verifier}], function (error, result) {
      if (error || !result) {
        callback && callback(
          error || new Error("Geen resultaat van changePassword."));
      } else {
        callback && callback();
      }
    });
...
Accounts.forgotPassword = function(options, callback) {
  if (!options.email)
    throw new Error("options.email moet doorgegeven worden.");
