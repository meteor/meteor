...
Meteor.publish("livedata_server_test_sub_context_async", async function (connectionId, userId) {
    await sleep(50)
    var callback = onSubscription[connectionId];
    ...
    function (test, onComplete) {
        makeTestConnection(
            ...
            function (clientConn, serverConn) {
                ...
                clientConn.subscribe("livedata_server_test_sub_context_async", serverConn.id, userId);
            }
        );
    }
);
...
Tinytest.addAsync(
    "livedata server - async publish object",
    function (test, onComplete) {
        makeTestConnection(
            test,
            function (clientConn, serverConn) {
...
Tinytest.addAsync(
    "livedata server - async publish cursor",
    function (test, onComplete) {
        makeTestConnection(
            test,
            (clientConn, serverConn) => {
                const remoteCollection = new Mongo.Collection("names",{connection: clientConn})
