 ...
 * Use this inside methods or publications traced with { otel: true } to access the span.
 *
 ...
 *
 *     span?.setAttribute('task.priority', data.priority);
 *     return await TasksCollection.insertAsync(data);
 *   }
 * }, { otel: true });
 */
 ...
 * Safe to call even if no span is active (will be a no-op).
 *
 * @param {string} name - Event name
 * @param {Object} attributes - Optional event attributes
 *
 * @example
 * Meteor.methods({
 ...
 *     addEvent('payment.complete', { paymentId: result.id, amount: result.amount });
 *
 *     return result;
 *   }
 * }, { otel: true });
 ...
 * Safe to call even if no span is active (will be a no-op).
 *
 * @param {Object} attributes - Object with key-value pairs to set as attributes
 *
 * @example
 * Meteor.methods({
 *   async 'tasks.create'(data) {
 ...
 * Safe to call even if no span is active (will be a no-op).
 * Note: This does NOT set the span status to error - use setSpanError for that.
 *
 * @param {Error} error - The exception to record
 *
 * @example
 * Meteor.methods({
 *   async 'tasks.create'(data) {
 ...
 *       // Handle error gracefully without failing the span
 *     }
 *     return await TasksCollection.insertAsync(data);
 *   }
 * }, { otel: true });
 ...
 * Also records the exception if provided.
 * Safe to call even if no span is active (will be a no-op).
 *
 ...
 *   async 'tasks.create'(data) {
 *     if (!data.title) {
 *       setSpanError('Missing required field: title');
 *       throw new Meteor.Error('invalid-data', 'Title is required');
 *     }
 *     return await TasksCollection.insertAsync(data);
 *   }
 * }, { otel: true });
 */
export function setSpanError(messageOrError) {
  const span = trace.getSpan(context.active());
  if (span) {
 ...
 * The child span will be automatically linked to the parent span.
 *
 * @param {string} name - Span name
 * @param {Object} attributes - Optional span attributes
 * @returns {Object} Span handle with methods: addEvent, setAttribute, setAttributes, end, fail
 *
 * @example
 * Meteor.methods({
 *   async 'orders.process'(orderId) {
 *     // Create a child span for a sub-operation
 *     const paymentSpan = createChildSpan('process.payment', { orderId });
 *
 *     try {
 *       paymentSpan.addEvent('charging.start');
 *       const result = await chargeCustomer(orderId);
 *       paymentSpan.setAttribute('payment.id', result.paymentId);
 *       paymentSpan.addEvent('charging.complete');
 *       paymentSpan.end();
 *       return result;
 *     } catch (error) {
 *       paymentSpan.fail(error);
 *       throw error;
 *     }
 *   }
 * }, { otel: true });
...
export function createChildSpan(name, attributes = {}) {
  const tracer = trace.getTracer('meteor.custom');
  ...
  return {
     ...
     * Child spans created inside will be linked to this span.
     */
    run(fn) {
      return context.with(spanContext, fn);
    },
