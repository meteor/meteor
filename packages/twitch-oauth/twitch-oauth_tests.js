Tinytest.addAsync(
  'twitch-oauth - run service oauth with mocked flow as expected',
  async function (test) {
    const oauthMock = mockBehaviours(OAuth, {
      _fetch: () => Promise.resolve({ json: () => ({ access_token: 'testToken', data: [{ id: '123', login: 'testuser', display_name: 'Test', email: 'test@example.com' }] })}),
    });
