...
_.extend(Meteor._LivedataSubscription.prototype, {
  ...
  _ensureObject: function (collection_name, id) {
    var self = this;
    if (!self.pending_data[collection_name])
      self.pending_data[collection_name] = {};
    if (!self.pending_data[collection_name][id])
      self.pending_data[collection_name][id] = {};
    return self.pending_data[collection_name][id];
  },

  set: function (collection_name, id, attributes) {
    var self = this;
    var obj = self._ensureObject(collection_name, id);
