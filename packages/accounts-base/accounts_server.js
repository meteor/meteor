...
//Let’s get started with the client.
  ...
  //Instead of using [`Meteor.loginWithPassword()`] you have to do a login method call with a different set of parameters. 
  //This allows us to recognise from the server if client intends to authenticate with LDAP. 
  //Make sure that you don’t pass a password option as part of the login request, otherwise the accounts-password login handler will throw an error.
 ...
 //Next I will tell you how to create and register the LDAP authentication handler in three parts.
  ...
  //After the library imports, options for the LDAP authentication are defined. 
  //Instead of connecting the LDAP client to a real LDAP directory I’ve used the public available directory of Forum Systems: [Online LDAP Test Server](http://www.forumsys.com/tutorials/integration-how-to/ldap/online-ldap-test-server/). 
  //Following up the function header of the authentication method and a suspicious object is declared. 
  //You might haven’t seen or read about the Future fiber yet. As you might know Meteor doesn’t like async code the same as Node does or you came along a situation where your asynchronous code didn’t work as expected. 
  //To keep it short, the authentication handler request code must be run synchronous and the Future fiber helps us running asynchronous code.
  ...
  //Now comes probably the most difficult part. The body of our auth method tells if the LDAP credentials are valid by binding and unbinding the LDAP user with the LDAP directory. 
  //Any misbehaviour results in the return value [false]. 
  //An important line to point out here is the return statement of the [`ldapAuth`] object which is also assigned with a new [`profile`] property. 
  //In case of successful authentication we will use this property to create a new Meteor user in the users collection in the next step.
  ...
  //Finally, in case of successful LDAP user check, a collection lookup finds out wether the authenticated users is already in the database and if not creates a new entry. 
  //As you can see the [`profile`] property of the [`ldapAuth`] object is now used as a parameter. 
  //To make sure that user is authenticated after a browser refresh you have to create a token and store it. 
  //The return object contains the user identity and the login token.
  //Of course you can adapt this example and use it to authenticate against another provider. 
  //Other accounts packages such as [`accounts-facebook`] and the [`accounts-password`] work almost the same way.
