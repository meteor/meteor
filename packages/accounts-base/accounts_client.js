...
export class AccountsClient extends AccountsCommon {
  ...
  // Methods for storing a login token and user id in the
  // browser's localStorage facility. It polls local storage every
  // few seconds to synchronize login state between multiple tabs
  // in the same browser.
  ...
  // Semi-internal API. Call this function to re-enable auto login
  // after if it was disabled at startup.
  _enableAutoLogin() {
 ...
 * @locus Client
 * @param {String} [token] The Meteor access token
 * @param {Function} [callback] Optional callback. Called with no arguments on success, or with a single `Error` argument on failure.
 * @importFromPackage meteor
 */
