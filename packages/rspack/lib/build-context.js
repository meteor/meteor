...
const MANAGED_EXTERNAL_IMPORTS_PATTERN =
  /^(?:function\s+lazyExternalImports\d+|\/\/\s*\(function\s+(?:eagerExternalImports\d+|lastImports(?:\d+)?))\s*\(\)\s*\{/m;
...
export function bumpServerRuntimeBuildId() {
  ...
  try {
    ...
    const contentsWithoutMarker = contents.replace(
      SERVER_RUNTIME_BUILD_ID_PATTERN,
      '',
    );
    const managedImportsMatch = MANAGED_EXTERNAL_IMPORTS_PATTERN.exec(
      contentsWithoutMarker,
    );
    const nextContents = managedImportsMatch
      ? [
          contentsWithoutMarker
            .slice(0, managedImportsMatch.index)
            .trimEnd(),
          marker,
          '',
          contentsWithoutMarker.slice(managedImportsMatch.index),
        ].join('\n')
      : `${contentsWithoutMarker.trimEnd()}\n${marker}\n`;
