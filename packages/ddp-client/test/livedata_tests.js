...
testAsyncMulti("livedata - isAsync call", [
  async function (test) {
    let isAsyncCall;
    Meteor.methods({
      isCallAsync: function () {
        isAsyncCall = Meteor.isAsyncCall();
      },
    });
    Meteor.call("isCallAsync");
    test.equal(isAsyncCall, false);

    await Meteor.callAsync("isCallAsync");
    test.equal(isAsyncCall, true);
  },
]);
