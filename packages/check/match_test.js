...
Tinytest.add("check - check throw all errors deeply nested", (test) => {
  ...
  const pattern = {
    ...
    any: Match.Any,
    maybe: {
      m: Match.Maybe(Number),
      a: [Match.Maybe(Number)],
      b: Match.Maybe([{ x: String, y: Number }]),
    },
    opt: {
      m: Match.Optional(Number),
      a: [Match.Optional(Number)],
      b: Match.Optional([{ x: String, y: Number }]),
    },
    int: { i: Match.Integer, a: [Match.Integer], b: [{ x: Match.Integer, y: Number }] },
    oneOf: {
      f: Match.OneOf(Number, Boolean),
      a: [Match.OneOf(Boolean, Function)],
      b: [{ x: Match.OneOf(String, Number), y: Match.OneOf(Boolean, null) }],
    },
    where: {
      w: Match.Where((x) => {
        check(x, String);
  ...
  test.equal(
    ...
    [
      `Match error: Missing key 'another' in field embedded`,
      `Match error: Missing key 'missing1'`,
      `Match error: Missing key 'missing2'`,
    ],
  );
  error.every((e) => test.instanceOf(e, Match.Error));
  test.isFalse(Match.test(value, pattern));
...
Tinytest.add("check - Match error path", (test) => {
  ...
  match(
    [[[{ foo: "something" }, { foo: "here" }], [{ foo: "asdf" }]], [[{ foo: 123 }]]],
    [[[{ foo: String }]]],
    "[1][0][0].foo",
  );
...
Tinytest.add("check - Match error message", (test) => {
  ...
  match(
    testInstanceChild,
    TestInstanceParent,
    `Expected ${TestInstanceParent.name || "particular constructor"}`,
  );
...
Tinytest.add(
  "check - Match methods that return class instances can be called as " + "constructors",
  (test) => {
