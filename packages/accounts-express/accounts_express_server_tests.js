...
if (Meteor.isServer) {
  ...
  Tinytest.addAsync("accounts-express - createAuthMiddleware - setup test routes", async (test) => {
    ...
    WebApp.handlers.get(
      "/api/express-test-request-forward",
      createAuthMiddleware({ required: true }),
      async (req, res) => {
        ...
        const innerResponse = await Meteor.fetch(
          Meteor.absoluteUrl("api/express-test-request-echo"),
          { auth: true },
        );
  ...
  Tinytest.addAsync(
    "accounts-express - createAuthMiddleware - unauthenticated requests",
    async (test) => {
      // Make request without auth token to required auth route
      const requiredRes = await Meteor.fetch(Meteor.absoluteUrl("api/express-test-auth"), {
        auth: false,
      });
      ...
      const optionalRes = await Meteor.fetch(Meteor.absoluteUrl("api/express-test-auth-optional"), {
        auth: false,
      });
  ...
  Tinytest.addAsync(
    "accounts-express - createAuthMiddleware - valid authentication",
    async (test) => {
      ...
      try {
        ...
        const optionalRes = await fetchWithToken(
  ...
  Tinytest.addAsync(
    "accounts-express - createAuthMiddleware - invalid authentication",
    async (test) => {
      // Test with invalid token on required auth route
      const invalidRes = await fetchWithToken(
      ...
      const nonexistentRes = await fetchWithToken(
        Meteor.absoluteUrl("api/express-test-auth"),
        token,
      );
      ...
      const invalidOptRes = await fetchWithToken(
        Meteor.absoluteUrl("api/express-test-auth-optional"),
        "invalid-token",
      );
      ...
      const malformedOptRes = await Meteor.fetch(
        Meteor.absoluteUrl("api/express-test-auth-optional"),
        {
          headers: { Authorization: "NotBearer token" },
          auth: false,
      ...
      const nonexistentOptRes = await fetchWithToken(
        Meteor.absoluteUrl("api/express-test-auth-optional"),
        token,
      );
  ...
  Tinytest.addAsync(
    "accounts-express - createAuthMiddleware - token/session mismatch",
    async (test) => {
      ...
      try {
        ...
        const cookieRes = await fetchWithCookie(
          Meteor.absoluteUrl("api/express-test-auth"),
          user2.token,
        );
  ...
  Tinytest.addAsync(
    "accounts-express - createAuthMiddleware - middleware stacking",
    async (test) => {
      const { userId, token } = await createUserWithToken();
      ...
      try {
        ...
        const response = await fetchWithToken(
          Meteor.absoluteUrl("api/express-test-auth/stacked"),
          token,
        );
        test.equal(response.status, 200);
        ...
        const prefixRes = await fetchWithToken(
          Meteor.absoluteUrl("api/express-test-auth/prefix"),
          token,
        );
  ...
  Tinytest.addAsync(
    "accounts-express - createAuthMiddleware - cookie-only authentication",
    async (test) => {
      const { userId, token } = await createUserWithToken();
      ...
      try {
        ...
        const optionalRes = await fetchWithCookie(
          Meteor.absoluteUrl("api/express-test-auth-optional"),
          token,
        );
        test.equal(optionalRes.status, 200);
  ...
  Tinytest.addAsync(
    "accounts-express - createAuthMiddleware - concurrency and isolation",
    async (test) => {
      const user1 = await createUserWithToken();
      const user2 = await createUserWithToken();
  ...
  for (const { name, fetchFn } of fetchTestCases) {
    ...
    Tinytest.addAsync(
      `accounts-express - ${name} - no auth options uses raw fetch`,
      async (test) => {
        const response = await fetchFn(Meteor.absoluteUrl("api/express-test-request-echo"));
        const data = await response.json();
  ...
  Tinytest.addAsync("accounts-express - fetch auth - setup forwarding routes", async (test) => {
    ...
    WebApp.handlers.get(
      "/api/express-test-package-fetch-forward",
      createAuthMiddleware({ required: true }),
      async (req, res) => {
        const innerResponse = await pkgFetch(Meteor.absoluteUrl("api/express-test-request-echo"), {
          auth: true,
        });
        const innerData = await innerResponse.json();
  ...
  Tinytest.addAsync(
    "accounts-express - cookie roundtrip - set cookie then authenticate via Meteor.fetch",
    async (test) => {
      const { userId, token } = await createUserWithToken();
  ...
  Tinytest.addAsync(
    "accounts-express - cookie roundtrip - expired cookie is rejected",
    async (test) => {
      const { userId, token } = await createUserWithToken();
  ...
  Tinytest.addAsync(
    "accounts-express - createAuthFetch - same-origin URL gets context token",
    async (test) => {
      const { wrapped, capture } = buildCapturingFetch();

      await _CurrentEndpointInvocation.withValue({ userId: "u1", loginToken: "context-tok" }, () =>
  ...
  Tinytest.addAsync(
    "accounts-express - createAuthFetch - cross-origin URL does NOT leak context token",
    async (test) => {
      const { wrapped, capture } = buildCapturingFetch();
      ...
      test.isFalse(
        capture.headers.has("Authorization"),
  ...
  Tinytest.addAsync(
    "accounts-express - createAuthFetch - explicit token attaches even cross-origin",
    async (test) => {
      const { wrapped, capture } = buildCapturingFetch();
  ...
  Tinytest.addAsync(
    "accounts-express - createAuthFetch - unparseable URL fails closed",
    async (test) => {
      const { wrapped, capture } = buildCapturingFetch();
  ...
  Tinytest.addAsync(
    "accounts-express - createAuthFetch - no auth opt-in skips context token",
    async (test) => {
      const { wrapped, capture } = buildCapturingFetch();
  ...
  Tinytest.addAsync(
    "accounts-express - createAuthMiddleware - optional sets req.userId=null on no token",
    async (test) => {
      const res = await Meteor.fetch(Meteor.absoluteUrl("api/express-test-auth-optional"), {
        auth: false,
      });
  ...
  Tinytest.addAsync(
    "accounts-express - createAuthMiddleware - optional sets req.userId=null on invalid token",
    async (test) => {
      ...
      test.isNull(
        data.reqUserId,
        "req.userId should be null when the token does not resolve to a user",
      );
    },
  );

  Tinytest.addAsync(
    "accounts-express - createAuthMiddleware - optional sets req.userId=null on expired token",
    async (test) => {
      const { userId, token } = await createUserWithToken();
  ...
  Tinytest.addAsync(
    "accounts-express - createAuthMiddleware - cookie value containing = is preserved",
    async (test) => {
      const { userId, token } = await createUserWithToken();
  ...
  Tinytest.addAsync(
    "accounts-express - handleFetch - auth: undefined falls back to rawFetch",
    async (test) => {
      let rawCalled = false;
      ...
      try {
        ...
        test.isFalse(
          meteorFetchCalled,
          "auth-wrapped Meteor.fetch should NOT be called when auth is undefined",
        );
      } finally {
  ...
  Tinytest.addAsync(
    "accounts-express - handleFetch - explicit auth: false routes through Meteor.fetch",
    async (test) => {
      let meteorFetchCalled = false;
      const originalMeteorFetch = Meteor.fetch;
      Meteor.fetch = async () => {
      ...
      try {
        ...
        test.isTrue(
          meteorFetchCalled,
          "auth: false is still an explicit signal — route through Meteor.fetch",
        );
      } finally {
        Meteor.fetch = originalMeteorFetch;
      }
  ...
  Tinytest.addAsync(
    "accounts-express - createAuthFetch - does not clobber caller-supplied Authorization",
    async (test) => {
      const { wrapped, capture } = buildCapturingFetch();
  ...
  Tinytest.addAsync(
    "accounts-express - cookie roundtrip - cleared cookie results in unauthenticated",
    async (test) => {
      const { userId, token } = await createUserWithToken();
  ...
  Tinytest.addAsync(
    "accounts-express - aeFetch - explicit token attaches by default",
    async (test) => {
      const { userId, token } = await createUserWithToken();
  ...
  Tinytest.addAsync(
    "accounts-express - aeFetch - context token forwards by default",
    async (test) => {
      const { fetch: pkgFetch } = require("meteor/fetch");

      // Dedicated route to avoid re-registering an existing path.
      WebApp.handlers.get(
        "/api/express-test-aefetch-forward",
        createAuthMiddleware({ required: true }),
        async (req, res) => {
