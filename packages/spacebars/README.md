...
1. If the items are objects with an `_id` field, use the value of that field to match items
2. If the items are objects with no `_id` field, use the index in the array to match items. In this case, appends are fast but prepends are slower.
3. If the items are numbers or strings, use their value as the matching key.

Regardless of the strategy used, the keys are deduplicateded so that arrays with repeating items work fine.
