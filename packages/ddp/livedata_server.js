...
_.extend(Session.prototype, {
  ...
  _startSubscription: function (handler, subId, params, name) {
    ...
    var sub = new Subscription(self, handler, {
 ...
 * @instanceName this
 */
var Subscription = function (session, handler, options) {
  var self = this;
  ...
  self._subscriptionId = options.subscriptionId;
  ...
  // a set of sub-subscriptions created by self.publishChild();
  self._children = [];

  // the reverse of the above. We need to ensure we stop when the parent does
  self._parent = options.parent;
  self._parent && self._parent.onStop(function() {
    self.stop();
  });

  // remember if we are ready.
  self._nodeReady = false;
  // if we and and all children are ready
  self._ready = false;
...
_.extend(Subscription.prototype, {
  ...
  _recreate: function () {
    var self = this;
    return new Subscription(self._session, self._handler, {
  ...
  _checkTreeReady: function() {
    var self = this;
    var allChildrenReady = _.all(self._children, function(child) {
      return child._treeReady;
    });
    self._treeReady = (allChildrenReady && self._nodeReady);
    if (self._treeReady) { 
      if (self._parent) {
        self._parent._checkTreeReady();
      } else {
        if (!self._subscriptionId)
  ...
  ready: function () {
    ...
    self._nodeReady = true;
    self._checkTreeReady();
  },
  
  /**
   * @summary Call inside the publish function. Creates a anonymous child subscription that is managed separately to this subscription.
   * @locus Server
   * @memberOf Subscription
   * @instance
   */
  publishChild: function(handler /*, params */) {
    var self = this;
    var child = new Subscription(self._session, handler, { 
      parent: self,
