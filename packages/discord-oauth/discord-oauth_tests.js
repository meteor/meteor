Tinytest.addAsync(
  'discord-oauth - run service oauth with mocked flow as expected',
  async function (test) {
    const oauthMock = mockBehaviours(OAuth, {
      _fetch: () => Promise.resolve({ json: () => ({ access_token: 'testToken', id: '123', username: 'testuser', global_name: 'Test', email: 'test@example.com', verified: true })}),
    });
