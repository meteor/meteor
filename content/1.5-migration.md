...
Previously, Meteor would automatically assume that any `MAIL_URL` which used port 465 or 587 was meant to be a secure scheme and automatically change `smtp://` to `smtps://` as necessary.  However, this is not always desired, and
...
Again, generally speaking, this applies to applications whose `MAIL_URL` already includes `:465` or `:587`.