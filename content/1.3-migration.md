...
As mentioned in the [breaking changes](#breaking-changes), the `react` Atmosphere package now expects you to install various React NPM packages in your application (read the [Using Packages](using-packages.html) article for more details about how to manage your NPM dependencies):
...
**However**, we recommend that you should not use the `react` or `react-runtime` Atmosphere packages and instead use React directly from NPM (for more detail, see the [React article](react.html) of the guide). In an existing app, you can run:
