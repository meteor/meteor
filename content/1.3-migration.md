...
As mentioned in the [breaking changes](#breaking-changes), the React Atmosphere package now "peer" depends on the React npm packages (see the [instructions from package authors](#packages) above for more details on this). If you are using the `react` Atmosphere package (and thus the `react-runtime` package), this means you are required to `npm install` the following set of NPM packages into your app (read the [Using Packages](using-packages.html) article for more details about how to manage your NPM dependencies):

```
npm install --save-dev react react-dom react-addons-transition-group \
...
<h2 id="react-meteor-data">Data access in React</h2>
