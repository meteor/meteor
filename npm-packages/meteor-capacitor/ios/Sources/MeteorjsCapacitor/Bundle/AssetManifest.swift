...
// AssetManifest.swift
//
// Handles parsing and managing the asset manifest that describes
// all files in a Meteor app bundle and their metadata.
//
...
struct AssetManifest {
    ...
    init(data: Data) throws {
        ...
        do {
            ...
            throw WebAppError.invalidAssetManifest(
                reason: "Error parsing asset manifest", underlyingError: error)
        }

        if let format = JSON["format"] as? String, format != "web-program-pre1" {
            throw WebAppError.invalidAssetManifest(
                reason: "The asset manifest format is incompatible: \(format)", underlyingError: nil)
        }

        guard let version = JSON["version"] as? String else {
            throw WebAppError.invalidAssetManifest(
                reason: "Asset manifest does not have a version", underlyingError: nil)
        }

        self.version = version

        guard let cordovaCompatibilityVersions = JSON["cordovaCompatibilityVersions"] as? JSONObject,
              let cordovaCompatibilityVersion = cordovaCompatibilityVersions["ios"] as? String
        else {
            throw WebAppError.invalidAssetManifest(
                reason: "Asset manifest does not have a cordovaCompatibilityVersion", underlyingError: nil)
        }
